...
describe('edit_document MCP tool', () => {
  test('describes the optional offset precision contract', () => {
    ...
    expect(DESCRIPTION).toContain('links({ kind: "suggest"');
  });
  ...
  test('surfaces the server field-level detail alongside the title (parity with write_document)', async () => {
    mockErrorEnvelope = {
      status: 400,
      body: {
        ...
        status: 400,
        detail: 'find: String must contain at least 1 character(s)',
      },
    };
    ...
    expect(text).toContain('find: String must contain at least 1 character(s)');
  });

  test('includes route-only previewUrl + source in structuredContent when resolver resolves', async () => {
...
describe('edit_document — autoOpen field on PreviewAttachWarning', () => {
